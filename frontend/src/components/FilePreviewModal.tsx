...
const FilePreviewModal: React.FC<Props> = ({ resource, visible, onClose }) => {
  ...
  useEffect(() => {
    ...
    // Office 走 Office Online（GitHub Raw），不需要后端代理
    if (isOffice) {
      setLoading(false);
      setViewerError(false);
      return;
  ...
  const officeUrl = isOffice ? `${GITHUB_RAW}/${resource.path}` : '';
        ...
        return (
          <div style={{ textAlign: 'center', padding: 60 }}>
            ...
            <p style={{ fontSize: 12, color: '#94A3B8' }}>
              文件可能大于 50MB 未同步到 GitHub，或网络问题
            </p>
            <div style={{ marginTop: 20, display: 'flex', justifyContent: 'center', gap: 12 }}>
              <Button type="primary" onClick={() => window.open(officeUrl, '_blank')}
                style={{ background: '#00A65E' }}>在新窗口打开</Button>
              <a href={officeUrl} download={resource.name}>
