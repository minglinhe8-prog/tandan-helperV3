...
API_RESOURCES_URL = f"{API_BASE_URL}/api/resources"
API_FAVORITES_URL = f"{API_BASE_URL}/api/favorites"
API_HISTORY_URL = f"{API_BASE_URL}/api/history"
USE_API = os.getenv("USE_API", "true").lower() == "true"  # 可回退到本地模式

# ========== Session State 初始化（认证） ==========
if "access_token" not in st.session_state:
    st.session_state.access_token = None
if "current_user" not in st.session_state:
    st.session_state.current_user = None
if "api_resource_map" not in st.session_state:
    st.session_state.api_resource_map = {}  # path -> resource_id 映射
...
def api_login(username: str, password: str):
    try:
        ...
        if response.status_code == 200:
            ...
            me = requests.get(f"{API_AUTH_URL}/me", headers=get_headers())
...
def api_search(keywords: Dict):
    """调用后端搜索资源，返回 [(id, category, Path), ...]"""
    try:
        params = {"size": 100, "page": 1}
        if keywords.get("grades"):
            params["grade"] = ",".join(keywords["grades"])
        if keywords.get("subjects"):
            params["subject"] = ",".join(keywords["subjects"])
        if keywords.get("course_type") and keywords["course_type"] != "全部":
            params["course_type"] = keywords["course_type"]
        if keywords.get("semester") and keywords["semester"] != "全部":
            params["semester"] = keywords["semester"]
        if keywords.get("teacher") and keywords.get("teacher").strip():
            params["teacher"] = keywords["teacher"].strip()
        if keywords.get("keyword"):
            params["keyword"] = keywords["keyword"]

        response = requests.get(API_RESOURCES_URL, headers=get_headers(), params=params)
        if response.status_code == 200:
            data = response.json()
            ...
            for item in items:
                ...
                results.append((rid, item["category"], file_path))
                st.session_state.api_resource_map[str(file_path)] = rid
            return results
        else:
...
def api_favorite_list():
    """获取收藏的资源列表"""
    try:
        response = requests.get(API_FAVORITES_URL, headers=get_headers())
        if response.status_code == 200:
            items = response.json()
            results = []
            for item in items:
...
def api_favorite_toggle(file_path: Path):
    """切换收藏状态，返回是否已收藏"""
    path_str = str(file_path)
    rid = st.session_state.api_resource_map.get(path_str)
    if not rid:
        st.warning("资源ID未找到，请刷新页面重试")
        return False
    try:
        # 检查当前状态：尝试取消收藏
        del_resp = requests.delete(f"{API_FAVORITES_URL}/{rid}", headers=get_headers())
        if del_resp.status_code == 200:
            return False  # 成功取消，当前未收藏
        # 未收藏则添加
        add_resp = requests.post(f"{API_FAVORITES_URL}/{rid}", headers=get_headers())
        return add_resp.status_code in [200, 201]
    except Exception as e:
        st.error(f"收藏操作失败: {e}")
        return False

def api_is_favorite(file_path: Path):
    """检查是否已收藏（简单判断：从本地缓存）"""
    return "fav_check" not in st.session_state or file_path not in st.session_state.get("fav_list", [])

def api_add_history(resource_id: int):
    try:
        requests.post(f"{API_HISTORY_URL}/{resource_id}", headers=get_headers())
    except Exception:
        pass

def api_get_history():
    try:
        response = requests.get(API_HISTORY_URL, headers=get_headers())
        if response.status_code == 200:
            items = response.json()
            results = []
            for item in items:
                file_path = BASE_DIR / item["path"]
                results.append((item["id"], item["category"], file_path))
            return results
        return []
    except Exception:
        return []
...
def resource_card(category: str, file_path: Path, index: int, resource_id: int = 0):
    ext = file_path.suffix.lower()
    thumb = get_thumbnail(file_path)

    # 收藏状态判断
    if USE_API and st.session_state.access_token and resource_id:
        # API模式：简单切换逻辑（每次点击切换）
        fav_state_key = f"fav_state_{resource_id}"
        if fav_state_key not in st.session_state:
            st.session_state[fav_state_key] = False  # 初始未收藏，点击后切换
        fav = st.session_state[fav_state_key]
    else:
    ...
    with st.container():
        ...
        with col_a:
            ...
            if USE_API and st.session_state.access_token and resource_id:
                api_add_history(resource_id)
            else:
                add_to_history(file_path)
        with col_b:
            fav_btn_key = f"fv_{index}_{file_path.name}_{resource_id}"
            if st.button(("★" if fav else "☆"), key=fav_btn_key, use_container_width=True):
                if USE_API and st.session_state.access_token and resource_id:
                    api_favorite_toggle(file_path)
                    st.session_state[fav_state_key] = not st.session_state.get(fav_state_key, False)
                else:
...
def main():
    ...
    with st.sidebar:
        ...
        if USE_API:
            if st.session_state.current_user:
                st.success(f"👤 {st.session_state.current_user['username']} ({st.session_state.current_user['role']})")
                if st.button("退出登录", use_container_width=True):
                    st.session_state.access_token = None
                    st.session_state.current_user = None
                    st.session_state.api_resource_map = {}
                    st.rerun()
            else:
                with st.expander("🔐 登录", expanded=True):
                    login_user = st.text_input("用户名", placeholder="输入用户名", key="login_user")
                    login_pass = st.text_input("密码", type="password", placeholder="输入密码", key="login_pass")
                    if st.button("登录", use_container_width=True, type="primary"):
                        result = api_login(login_user, login_pass)
                        if result["success"]:
                            st.success("登录成功！")
                            st.rerun()
                        else:
        ...
        with st.expander("⚡ 快捷场景", expanded=False):
            ...
            with c2:
                if st.button("📋 全部课表", key="quick_all"):
                    if USE_API and st.session_state.access_token:
                        st.session_state.results = api_search({"category": "课表"})
                    else:
                        raw = scan_course_table({"grades":[],"semester":"全部","course_type":"全部"})
                        st.session_state.results = [(0, c, p) for c, p in raw]
                    st.rerun()
            ...
            if st.button("🔍 搜索资源", use_container_width=True, type="primary"):
                ...
                if USE_API:
                    if not st.session_state.access_token:
                        ...
                        st.session_state.results = api_search(keywords)
                        st.session_state.compare_list = []
                else:
                    raw = search_all(keywords)
                    st.session_state.results = [(0, c, p) for c, p in raw]
                    st.session_state.compare_list = []
    ...
    if st.session_state.preview_file:
        ...
        category = None
        for rid, cat, f in st.session_state.results:
            if f == file_path:
                category = cat
                break
    ...
    if st.session_state.show_favorites:
        st.markdown("### ❤️ 我的收藏")
        if USE_API and st.session_state.access_token:
            favs = api_favorite_list()
        else:
            favs_raw = load_json(FAVORITES_FILE, [])
            favs = [(0, "收藏", Path(p)) for p in favs_raw if Path(p).exists()]
        if not favs:
            st.markdown("""
            ...
            for i, item in enumerate(favs):
                ...
                if f.exists():
                    with cols[i % 4]:
                        resource_card(cat, f, i, resource_id=rid)
    ...
    if not st.session_state.results:
        ...
        st.success(f"✅ 找到 {len(results)} 个资源 — 共 {len(set(c for _, c, _ in results))} 个分类")
        ...
        for rid, cat, path in results:
            grouped[cat].append((rid, path))
        ...
        for category, files in grouped.items():
            ...
            for i, item in enumerate(files):
                rid, f = item
                with cols[i % 4]:
                    resource_card(category, f, i, resource_id=rid)
